...
export class BleDevice implements IHealthDevice {
  ...
  private async enableItem(
    ...
    this.enabled[item.id] = this.nativeDevice.monitorCharacteristicForService(
      ...
      (error, characteristic) => {
        ...
        this.eventEmitter.emit(DATA_AVAILABLE_EVENT, {
          itemId: item.id,
          value: this.getValue(characteristic),
          itemName: item.name,
        });
      },
    );
  ...
  async fetch(): Promise<void> {
    ...
    this.items = (
      await Promise.all(
        (
          await this.nativeDevice.services()
        ).map(
          async service =>
            await Promise.all(
              (
                await service.characteristics()
              ).map<IHealthItem>(characteristic => {
                let c: any = {
...
async function getManufacturerDeviceFromId(
  nativeDevice: Device,
): Promise<IHealthDevice> {
  let custom = null;
  if (nativeDevice.serviceUUIDs) {
    ManufacturerMap.getManufacturers().forEach(manId => {
      ...
      if (
        ids.filter(id => nativeDevice.serviceUUIDs?.includes(id)).length > 0
      ) {
